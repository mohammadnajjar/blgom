    ...
    public function boot()
    {
        ...
        view()->composer('*', function ($view) {
            if (!Cache::has('recent_posts')) {
                ...
                $recent_posts = $recent_posts->whereHas('category', function ($query) {
                    $query->whereStatus(1);
                });
                $recent_posts = $recent_posts->whereHas('user', function ($query) {
                    $query->whereStatus(1);
                });
                $recent_posts = $recent_posts->wherePostType('post')->wherePostStatus(1)->orderBy('id', 'desc')
                    ->limit(5)->get();
            ...
            if (!Cache::has('recent_comments')) {
                $recent_comments = Comment::whereCommentStatus(1)->orderBy('id', 'desc')
                    ->limit(5)->get();
            ...
            if (!Cache::has('global_archives')) {
                $global_archives = Post::wherePostStatus(1)->orderBy('created_at', 'desc')
